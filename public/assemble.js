...
(function () {
  ...
  function openAssemble() {
    ...
    // Предупреждение о том, чем является результат.
    //
    // Это не юридическая формальность и не мелкий шрифт: сервис
    // действительно отдаёт черновик, который нужно читать и править.
    // Человек, который сдаст это не глядя, получит по заслугам — и
    // будет прав, если скажет, что его не предупредили.
    var warning = el('div', {
      border: '1px solid #8a7a3a', background: '#1a1608',
      padding: '8px', fontSize: '12px', lineHeight: '1.45',
      color: '#d8cf9a',
    });
    warning.appendChild(el('div', { fontWeight: 'bold', marginBottom: '4px' },
      '⚠ Это черновик, а не готовая работа'));
    warning.appendChild(el('div', {},
      'Текст написан ИИ. Проверьте факты, ссылки на законы и судебную '
      + 'практику — модель их помнит неточно. Прочитайте работу целиком '
      + 'и перепишите то, что звучит не по-вашему: научный руководитель '
      + 'задаёт вопросы по тексту.'));
